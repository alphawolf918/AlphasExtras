...
public class AcaciaLeaves extends BlockLeaves implements IShearable
{
	...
	public AcaciaLeaves(int par1)
	{
		super(par1);
		this.setTickRandomly(true);
		this.setCreativeTab(ModTabs.aiTab);
		this.setUnlocalizedName("acacialeaves");
			...
			int par3, int par4)
	{
		...
		else
		{
			...
			for (int l1 = -1; l1 <= 1; ++l1)
			{
				...
				{
					int j2 = par1IBlockAccess.getBiomeGenForCoords(
							par2 + i2,
			...
			return (i1 / 9 & 255) << 16 | (j1 / 9 & 255) << 8 | k1 / 9
					& 255;
		}
	}
			...
			int par5, int par6)
	{
		...
		if (par1World.checkChunksExist(par2 - j1, par3 - j1, par4 - j1,
				par2
						+ j1, par3 + j1, par4 + j1))
		{
			for (int k1 = -b0; k1 <= b0; ++k1)
			{
				for (int l1 = -b0; l1 <= b0; ++l1)
				{
					...
					{
						int j2 = par1World.getBlockId(par2 + k1,
								par3 + l1,
								par4 + i2);
						
						if (Block.blocksList[j2] != null)
						{
							Block.blocksList[j2].beginLeavesDecay(
									par1World,
			...
			Random par5Random)
	{
		if (!par1World.isRemote)
		{
			...
			if ((l & 8) != 0 && (l & 4) == 0)
			{
				...
				if (par1World.checkChunksExist(par2 - i1, par3 - i1, par4
						- i1,
						par2 + i1, par3 + i1, par4 + i1))
				{
					...
					for (l1 = -b0; l1 <= b0; ++l1)
					{
						for (i2 = -b0; i2 <= b0; ++i2)
						{
							for (j2 = -b0; j2 <= b0; ++j2)
							{
								k2 = par1World.getBlockId(par2 + l1, par3
										+ i2,
										par4 + j2);
								...
								if (block != null
										&& block.canSustainLeaves(
												par1World,
												par2 + l1, par3 + i2, par4
														+ j2))
								{
			...
			int par4, Random par5Random)
	{
				...
				&& par5Random.nextInt(15) == 1)
		{
			...
			par1World.spawnParticle("dripWater", d0, d1, d2, 0.0D, 0.0D,
					0.0D);
		}
	}
	...
	{
		return Blocks.acaciaSapling.blockID;
	}
	...
	public void registerIcons(IconRegister par1IconRegister)
	{
		this.blockIcon = par1IconRegister
				.registerIcon("awei:acacialeaves");
	}
