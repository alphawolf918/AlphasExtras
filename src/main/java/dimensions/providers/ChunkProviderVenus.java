...
public class ChunkProviderVenus implements IChunkProvider {
	...
	public ChunkProviderVenus(World par1World, long par2, boolean par4) {
		...
		NoiseGeneratorOctaves[] noiseGens = { noiseGen1, noiseGen2,
				noiseGen3,
		...
		for (int i1 = 0; i1 < b0; ++i1) {
			for (int j1 = 0; j1 < b0; ++j1) {
				for (int k1 = 0; k1 < b1; ++k1) {
					double d0 = 0.125D;
					double d1 = this.noiseArray[((i1 + 0) * l + j1 + 0)
							* b3
							+ k1 + 0];
					double d2 = this.noiseArray[((i1 + 0) * l + j1 + 1)
							* b3
							+ k1 + 0];
					double d3 = this.noiseArray[((i1 + 1) * l + j1 + 0)
							* b3
							+ k1 + 0];
					double d4 = this.noiseArray[((i1 + 1) * l + j1 + 1)
							* b3
							+ k1 + 0];
					double d5 = (this.noiseArray[((i1 + 0) * l + j1 + 0)
							* b3
							+ k1 + 1] - d1)
							* d0;
					double d6 = (this.noiseArray[((i1 + 0) * l + j1 + 1)
							* b3
							+ k1 + 1] - d2)
							* d0;
					double d7 = (this.noiseArray[((i1 + 1) * l + j1 + 0)
							* b3
							+ k1 + 1] - d3)
							* d0;
					double d8 = (this.noiseArray[((i1 + 1) * l + j1 + 1)
							* b3
							+ k1 + 1] - d4)
							* d0;
	...
	public void replaceBlocksForBiome(int par1, int par2,
		...
		for (int k = 0; k < 16; ++k) {
			for (int l = 0; l < 16; ++l) {
				BiomeGenBase biomegenbase = par4ArrayOfBiomeGenBase[l + k
						* 16];
	...
	public Chunk provideChunk(int par1, int par2) {
		...
		this.caveGenerator
				.generate(this, this.worldObj, par1, par2, abyte);
		this.ravineGenerator.generate(this, this.worldObj, par1, par2,
				abyte);
		
		if (this.mapFeaturesEnabled) {
			this.mineshaftGenerator.generate(this, this.worldObj, par1,
					par2,
					abyte);
			this.villageGenerator.generate(this, this.worldObj, par1,
					par2,
					abyte);
	...
	private double[] initializeNoiseField(double[] par1ArrayOfDouble,
		...
		ChunkProviderEvent.InitNoiseField event = new ChunkProviderEvent.InitNoiseField(
				this, par1ArrayOfDouble, par2, par3, par4, par5, par6,
				par7);
		MinecraftForge.EVENT_BUS.post(event);
		if (event.getResult() == Result.DENY)
		...
		double d0 = 684.412D;
		double d1 = 684.412D;
		this.noise5 = this.noiseGen5.generateNoiseOctaves(this.noise5,
				par2,
		...
		for (int k2 = 0; k2 < par5; ++k2) {
			for (int l2 = 0; l2 < par7; ++l2) {
				...
				float f3 = 0.0F;
				byte b0 = 2;
				BiomeGenBase biomegenbase = this.biomesForGeneration[k2
						+ 2
						+ (l2 + 2) * (par5 + 5)];
	 ...
	 */
	@Override
	public void populate(IChunkProvider par1IChunkProvider, int par2,
			int par3) {
		...
		if (this.mapFeaturesEnabled) {
			this.mineshaftGenerator.generateStructuresInChunk(
					this.worldObj,
					this.rand, par2, par3);
						...
						par2, par3, flag, LAKE))
		{
			...
			if (TerrainGen.populate(par1IChunkProvider, worldObj, rand,
					par2,
					par3, flag, LAVA) &&
					!flag && this.rand.nextInt(8) <= 2)
			{
				k1 = k + this.rand.nextInt(16) + 8;
				...
				{
					(new WorldGenLakes(Blocks.fuelLiquidBlock.blockID))
							.generate(
									this.worldObj, this.rand, k1, l1, i2);
				}
			}
	...
	public List getPossibleCreatures(
			EnumCreatureType par1EnumCreatureType,
	...
	public ChunkPosition findClosestStructure(World par1World,
		...
		return "Stronghold".equals(par2Str)
				...
				? this.strongholdGenerator.getNearestInstance(par1World,
						par3,
						par4, par5) : null;
	}
