package alphaitems.dimensions.teleporter;
...
public class TeleporterVenus extends Teleporter {
	...
	public boolean placeInExistingPortal(Entity par1Entity, double par2,
		...
		if (this.destinationCoordinateCache.containsItem(j1)) {
			...
			for (k1 = l - short1; k1 <= l + short1; ++k1) {
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					...
					for (int i2 = this.worldServerInstance
							.getActualHeight() - 1; i2 >= 0; --i2) {
						if (this.worldServerInstance
								.getBlockId(k1, i2, l1) == Blocks.venusPortal.blockID) {
							while (this.worldServerInstance.getBlockId(k1,
									i2 - 1, l1) == Blocks.venusPortal.blockID) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			...
			if (this.worldServerInstance.getBlockId(i - 1, j, k) == Blocks.venusPortal.blockID) {
				j2 = 2;
			}
			...
			if (this.worldServerInstance.getBlockId(i, j, k - 1) == Blocks.venusPortal.blockID) {
				j2 = 3;
			}
			...
			if (j2 > -1) {
				...
				boolean flag1 = !this.worldServerInstance.isAirBlock(i
						+ i3
						+ k3, j, k + j3 + l3)
						|| !this.worldServerInstance.isAirBlock(i + i3
								+ k3,
								j + 1, k + j3 + l3);
				boolean flag2 = !this.worldServerInstance.isAirBlock(i
						+ i3, j,
						k + j3)
						|| !this.worldServerInstance.isAirBlock(i + i3,
								j + 1,
				...
				if (flag1 && flag2) {
					...
					int i4 = k - l3;
					d4 -= l3;
					flag1 = !this.worldServerInstance.isAirBlock(k1 + i3
							+ k3,
							j, i4 + j3 + l3)
							|| !this.worldServerInstance.isAirBlock(k1
									+ i3
									+ k3, j + 1, i4 + j3 + l3);
					flag2 = !this.worldServerInstance.isAirBlock(k1 + i3,
							j, i4
									+ j3)
							|| !this.worldServerInstance.isAirBlock(k1
									+ i3,
		...
		for (k2 = 0; k2 < 4; ++k2) {
			for (i3 = 0; i3 < 4; ++i3) {
				for (l2 = -1; l2 < 4; ++l2) {
					...
					this.worldServerInstance.setBlock(k3, j3, i4, flag
							? Block.obsidian.blockID
							: Blocks.venusPortal.blockID, 0, 2);
				}
			}
		...
		if (par1 % 100L == 0L) {
			...
			while (iterator.hasNext()) {
				...
				if (portalposition == null
						|| portalposition.lastUpdateTime < j) {
					iterator.remove();
