...
public class WorldGenDungeonsVenus extends WorldGenerator
{
	public static final WeightedRandomChestContent[] field_111189_a = new WeightedRandomChestContent[] {
			...
			new WeightedRandomChestContent(Item.ingotIron.itemID, 0, 1, 4,
					10),
			new WeightedRandomChestContent(Item.bread.itemID, 0, 1, 1, 10),
			new WeightedRandomChestContent(Item.wheat.itemID, 0, 1, 4, 10),
			new WeightedRandomChestContent(Item.gunpowder.itemID, 0, 1, 4,
					10),
			new WeightedRandomChestContent(Item.silk.itemID, 0, 1, 4, 10),
			new WeightedRandomChestContent(Item.bucketEmpty.itemID, 0, 1,
					1, 10),
			new WeightedRandomChestContent(Item.appleGold.itemID, 0, 1, 1,
					1),
			new WeightedRandomChestContent(Item.redstone.itemID, 0, 1, 4,
					10),
			new WeightedRandomChestContent(Item.record13.itemID, 0, 1, 1,
					10),
			new WeightedRandomChestContent(Item.recordCat.itemID, 0, 1, 1,
					10),
			new WeightedRandomChestContent(Item.nameTag.itemID, 0, 1, 1,
					10),
			new WeightedRandomChestContent(Item.horseArmorGold.itemID, 0,
					1, 1, 2),
			new WeightedRandomChestContent(Item.horseArmorIron.itemID, 0,
					1, 1, 5),
			new WeightedRandomChestContent(Item.horseArmorDiamond.itemID,
					0, 1, 1, 1) };
			...
			int par4, int par5)
	{
		...
		for (k1 = par3 - l - 1; k1 <= par3 + l + 1; ++k1)
		{
			for (l1 = par4 - 1; l1 <= par4 + b0 + 1; ++l1)
			{
				...
				{
					Material material = par1World.getBlockMaterial(k1, l1,
							i2);
		...
		if (j1 >= 1 && j1 <= 5)
		{
					...
					for (i2 = par5 - i1 - 1; i2 <= par5 + i1 + 1; ++i2)
					{
								...
								&& i2 != par5 + i1 + 1)
						{
							par1World.setBlockToAir(k1, l1, i2);
								...
								&& !par1World.getBlockMaterial(k1, l1 - 1,
										i2).isSolid())
						{
							par1World.setBlockToAir(k1, l1, i2);
						}
								...
								.isSolid())
						{
									...
									&& par2Random.nextInt(4) != 0)
							{
								par1World.setBlock(k1, l1, i2,
										Blocks.venusCobble.blockID, 0,
										2);
							}
							else
							{
								par1World.setBlock(k1, l1, i2,
										Blocks.venusMossy.blockID, 0, 2);
							}
						}
					}
				}
				...
				while (true)
				{
					if (l1 < 3)
					{
						label101:
						{
							...
							int j2 = par5 + par2Random.nextInt(i1 * 2 + 1)
									- i1;
							...
							if (par1World.isAirBlock(i2, par4, j2))
							{
								...
								if (par1World.getBlockMaterial(i2 - 1,
										par4, j2).isSolid())
								{
									++k2;
								}
								...
								if (par1World.getBlockMaterial(i2, par4,
										j2 - 1).isSolid())
								{
									++k2;
								}
			...
			if (tileentitymobspawner != null)
			{
				tileentitymobspawner.getSpawnerLogic().setMobID(
						this.pickMobSpawner(par2Random));
			}
			else
			{
				System.err
