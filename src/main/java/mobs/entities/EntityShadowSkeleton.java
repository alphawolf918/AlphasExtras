...
public class EntityShadowSkeleton extends EntityMob implements
		IRangedAttackMob {
				...
				if (itemstack != null) {
					if (itemstack.isItemStackDamageable()) {
						...
						if (itemstack.getItemDamageForDisplay() >= itemstack
							...
							this.setCurrentItemOrArmor(Item.bow.itemID,
									null);
						}
					}
	...
	public void onDeath(DamageSource par1DamageSource) {
		...
		if (par1DamageSource.getSourceOfDamage() instanceof EntityArrow
			...
			if (d0 * d0 + d1 * d1 >= 2500.0D) {
				entityplayer
						.triggerAchievement(AchievementList.snipeSkeleton);
			}
		}
	}
	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		for (k = 0; k < j; ++k) {
			this.dropItem(Items.shadowBone.itemID, 1);
		}
	}
	...
	protected void dropRareDrop(int par1) {
		if (this.getSkeletonType() == 1) {
			this.entityDropItem(new ItemStack(Item.skull.itemID, 1, 1),
					0.0F);
		}
	}
			...
			EntityLivingData par1EntityLivingData)
	{
		...
		this.setCanPickUpLoot(this.rand.nextFloat() < 0.55F * this.worldObj
				.getLocationTensionFactor(this.posX, this.posY, this.posZ));
	...
	public void attackEntityWithRangedAttack(
		...
		EntityArrow entityarrow = new EntityArrow(this.worldObj, this,
				...
				14 - this.worldObj.difficultySetting * 4);
		...
		entityarrow.setDamage(par2 * 2.0F + this.rand.nextGaussian()
				* 0.25D
				+ this.worldObj.difficultySetting * 0.11F);
		
		if (i > 0) {
			entityarrow.setDamage(entityarrow.getDamage() + i * 0.5D
					+ 0.5D);
		}
