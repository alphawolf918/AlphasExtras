...
public class AEChestGenHooks extends ChestGenHooks
{
	...
	public static final WeightedRandomChestContent[] CTChestContents = new WeightedRandomChestContent[] {
			new WeightedRandomChestContent(Item.ingotIron.itemID, 0, 1, 5,
					10),
			new WeightedRandomChestContent(Item.ingotGold.itemID, 0, 1, 5,
					10),
			new WeightedRandomChestContent(Items.ampIngot.itemID, 0,
					1, 5,
					10),
			new WeightedRandomChestContent(Items.fuelIngot.itemID, 0, 1,
					5,
					10),
			new WeightedRandomChestContent(Items.spcItem.itemID, 0, 1, 5,
					10),
			new WeightedRandomChestContent(Items.enderShard.itemID, 0, 1,
					5,
					10), };
	
	private static final HashMap<String, AEChestGenHooks> chestInfo = new HashMap<String, AEChestGenHooks>();
	...
	private static void init()
	{
		...
		addInfo(CRYSTAL_TOWER,
				CTChestContents, 3, 7);
		
		ItemStack book = new ItemStack(Item.enchantedBook, 1, 0);
		WeightedRandomChestContent tmp = new WeightedRandomChestContent(
				book, 1, 1, 1);
		getInfo(CRYSTAL_TOWER).addItem(tmp);
	}
	
	static void addDungeonLoot(AEChestGenHooks dungeon, ItemStack item,
			int weight, int min, int max)
	{
		dungeon.addItem(new WeightedRandomChestContent(item, min, max,
				weight));
	}
	...
	{
		chestInfo.put(category, new AEChestGenHooks(category, items, min,
				max));
	}
	 ...
	 */
	public static AEChestGenHooks getInfo(String category)
	{
		if (!chestInfo.containsKey(category))
		{
			chestInfo.put(category, new AEChestGenHooks(category));
		}
	...
	public static ItemStack[] generateStacks(Random rand,
			ItemStack source, int min, int max)
	{
	...
	public AEChestGenHooks(String category)
	{
		super(category);
	...
	public WeightedRandomChestContent[] getItems(Random rnd)
	{
		...
		for (WeightedRandomChestContent orig : contents)
		{
			...
			if (item != null)
			{
				WeightedRandomChestContent n = item.getChestGenBase(this,
						rnd, orig);
				if (n != null)
				{
					ret.add(n);
	...
	public ItemStack getOneItem(Random rand)
	{
		...
		ItemStack[] stacks = AEChestGenHooks.generateStacks(rand,
