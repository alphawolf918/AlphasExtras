...
public class WorldGenShinestone extends WorldGenerator
{
	...
	public boolean generate(World par1World, Random par2Random, int par3,
			int par4, int par5)
	{
		int bid = par1World.getBlockId(par3, par4 + 1, par5);
		
		if (!par1World.isAirBlock(par3, par4, par5))
		{
			return false;
		}
		else if (bid != Blocks.venusRock.blockID
				&& bid != Blocks.venusSand.blockID)
		{
			return false;
		}
		else
			...
			for (int l = 0; l < 1500; ++l)
			{
				...
				if (par1World.getBlockId(i1, j1, k1) == 0)
				{
					...
					for (int i2 = 0; i2 < 6; ++i2)
					{
						...
						if (j2 == Blocks.shineStone.blockID)
						{
							++l1;
						}
					}
